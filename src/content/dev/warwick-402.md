...
#### 3.1 what can compiler approach help in Parallel computing?
...
- 3.2.1 data parallelism vs task parallelism - Task parallelism: multiple processors run different operations - Data parallelism: multiple processors run the same operation on
...
##### 3.6.1 techniques for synchronization
...
##### 3.6.6 difference between synchronization in Java and C
...
- buffered mode - sender uses explicitly defined buffer instead of sys buffer - communication is considered complete when 1. data copied to defined buffer(same as standard?) - blocking buffered send : MPI_Bsend - must attach buffer space using MPI_Buffer_attach/detach - the size should be no less than the value:
...
- Asymptotic analysis
  ...
  - disadvantage:
    ...
    - only tell the order, not actual time
...
- 1-D along y decomposition
  - modelling computation time
    ...
    - `Tcomp = tc _ N_(N/P)\*Z, for each subgrid, tc is the avg time of calculating a single grid point`
...
- 2-D
  - `Tcomp = tc _ N _ (N/P) \*Z same as 1D`
...
#### 5.5 Design philosophy of CPU and GPU
...
#### 7.3 two switching techniques and their performance models
